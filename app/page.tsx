...
export default function HomePage() {
  ...
  const seleccionarCantidad = (valor: 1 | 3) => {
    setCantidad(valor);
  };
  ...
  const handleComprar = () => {
    ...
    if (!cantidad) {
      alert("Selecciona una opción de compra.");
      return;
    }
    alert(
      `Compra: ${cantidad} número(s)\nNombre: ${nombre}\nTeléfono: ${telefono}\nCorreo: ${email}`
    );
  };
    ...
    <main className="min-h-screen bg-gradient-to-b from-rose-50 via-white to-sky-50 text-slate-800">
      <section className="max-w-6xl mx-auto px-4 md:px-6 py-8 md:py-14">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <h1 className="text-3xl md:text-6xl font-bold leading-tight">
            Ayúdanos con los gastos de{" "}
            <span className="text-rose-600">Consuelo</span>
          </h1>

          <p className="mt-5 text-base md:text-lg text-slate-600 leading-relaxed">
            Estamos realizando una rifa solidaria para apoyar los gastos médicos
            de nuestra hija. Cada aporte es una ayuda real para nuestra familia.
          </p>
        </div>
      </section>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5 md:p-8">
          ...
          <div className="space-y-4 mb-6">
            ...
            <p className="text-sm text-slate-500">
              Después del pago te mostraremos tus números en pantalla y también
              te los enviaremos a tu correo.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            >
              ...
              <div className="text-sm text-slate-500 mt-2">
                Se asigna automáticamente 1 número disponible
              </div>
            </button>

            <button
              type="button"
              onClick={() => seleccionarCantidad(3)}
              className={`w-full rounded-3xl border-2 p-5 md:p-6 text-left transition-all ${
                cantidad === 3
            ...
            >
              ...
              <div className="text-sm text-rose-600 mt-2">
                Mejor oferta · se asignan 3 números automáticos
              </div>
            </button>
          </div>
          ...
          <div className="mb-6 text-center">
            <span className="inline-block rounded-full bg-slate-100 px-4 py-2 text-sm font-medium text-slate-700">
              {cantidad === 1
                ? "Selección actual: 1 número por $2.000"
                : cantidad === 3
                ? "Selección actual: 3 números por $5.000"
          ...
          <button
            ...
            className="w-full mt-2 px-8 py-4 bg-rose-600 hover:bg-rose-700 active:bg-rose-800 text-white rounded-2xl font-bold text-lg transition"
          >
            {cantidad === 1
              ? "Pagar $2.000 con Mercado Pago"
              : cantidad === 3
              ? "Pagar $5.000 con Mercado Pago"
      ...
      <section className="max-w-6xl mx-auto px-4 md:px-6 py-8 md:py-12">
        <div className="flex items-center justify-between gap-4 mb-4">
          ...
          <div className="text-sm text-slate-500">Del 001 al 500</div>
        </div>
        ...
        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-10 gap-2">
          {numeros.map((n) => (
            <div
              key={n}
              className="p-3 bg-white rounded-xl text-center shadow-sm border border-slate-100 text-sm font-medium"
            >
      ...
      <section className="max-w-4xl mx-auto px-4 md:px-6 py-8 md:py-12">
        <div className="bg-slate-900 text-white p-6 rounded-3xl text-center">
          <p className="leading-relaxed">
            Esta rifa es de carácter solidario para apoyar los gastos médicos de
            Consuelo. Gracias por tu apoyo y por compartir esta ayuda.
          </p>
        </div>
      </section>
    </main>
